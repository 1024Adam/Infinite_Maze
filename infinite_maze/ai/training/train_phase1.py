...
def train_phase_1(steps: int = 500000,
    ...
    while steps_done < steps and not training_complete:
        ...
        if steps_done >= last_progress_step + progress_interval:
            ...
            if steps_done > 0:
                ...
                print(f"Current reward: {episode_stats['reward']:.2f} | Current score: {episode_stats.get('score', 0):.2f}")
                print(f"Exploration rate: {agent.epsilon:.3f} | Episode length: {episode_stats.get('length', 0)}")
        ...
        if next_eval_step > 0 and steps_done >= next_eval_step and (last_eval_step := getattr(agent, 'last_eval_step', -eval_interval)) < next_eval_step:
            ...
            print(f"Avg Reward: {eval_stats['avg_reward']:.2f} ± {eval_stats['std_reward']:.2f} | "
                 f"Avg Score: {eval_stats.get('avg_score', 0):.2f} ± {eval_stats.get('std_score', 0):.2f} | "
                 f"Avg Length: {eval_stats['avg_length']:.2f}")
            print(f"Collision Rate: {eval_stats.get('collision_rate', 0):.4f} | "
                 f"Path Efficiency: {eval_stats.get('path_efficiency', 0):.4f} | "
            ...
            # Use the metrics directly from our enhanced evaluation
            eval_stats_extended = {
                'avg_reward': eval_stats['avg_reward'],
                'forward_success_rate': eval_stats.get('forward_success_rate', 0.0),
                'collision_rate': eval_stats.get('collision_rate', 1.0)
            }
    ...
    print(f"Avg Score: {final_eval_stats.get('avg_score', 0):.2f} ± {final_eval_stats.get('std_score', 0):.2f}")
    print(f"Avg Episode Length: {final_eval_stats['avg_length']:.2f} ± {final_eval_stats['std_length']:.2f}")
    print(f"Collision Rate: {final_eval_stats.get('collision_rate', 0):.4f}")
    print(f"Path Efficiency: {final_eval_stats.get('path_efficiency', 0):.4f}")
    print(f"Forward Success Rate: {final_eval_stats.get('forward_success_rate', 0):.4f}")
    ...
    final_stats = {
        ...
        'path_efficiency': float(final_eval_stats.get('path_efficiency', 0)),
